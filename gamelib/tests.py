...
class BasicTests(unittest.TestCase):
    ...
    def test_simple_fields(self):
        ...
        self.assertEqual(
            5, game.get_resource(game.MP, 1), "My opponent should have 5 MP"
        )
        self.assertEqual(
            25, game.get_resource(game.SP, 1), "My opponent should have 25 SP"
        )
        self.assertEqual(
            0,
            game.turn_number,
            "The map does not have a turn_number, or we can't read it",
        )
    ...
    def test_spawning(self):
        game = self.make_turn_0_map()
        self.assertEqual(
            True, game.attempt_spawn("SI", [[13, 0]]), "We cannot spawn a soldier!"
        )
        self.assertEqual(
            False,
            game.attempt_spawn("SI", [[13, 13]]),
            "We can spawn a soldier in the middle of the map?!?!",
        )
        self.assertEqual(
            False,
            game.can_spawn("FF", [14, 14]),
            "Apparently I can place towers on my opponent's side",
        )
        self.assertEqual(
            True, game.attempt_spawn("DF", [[13, 6]]), "We cannot spawn a tower!"
        )
        self.assertEqual(
            2,
            game.attempt_spawn("SI", [[13, 0], [13, 0], [13, 5]]),
            "More or less than 2 units were spawned!",
        )
        self.assertEqual([("DF", 13, 6)], game._build_stack, "Build queue is wrong!")
        self.assertEqual(
            [("SI", 13, 0), ("SI", 13, 0), ("SI", 13, 0)],
            game._deploy_stack,
            "Deploy queue is wrong!",
        )
    ...
    def test_trivial_functions(self):
        ...
        self.assertEqual(
            1,
            game.game_map.distance_between_locations([0, 0], [0, -1]),
            "The distance between 0,0 and 0,-1 should be 1",
        )
        self.assertEqual(
            1,
            game.game_map.distance_between_locations([-1, -1], [-2, -1]),
            "The distance between -1,-1 and -2,-1 should be 1",
        )
        self.assertEqual(
            5,
            game.game_map.distance_between_locations([0, 0], [4, 3]),
            "The distance between 0,0 and 16,9 should be 5",
        )
        self.assertEqual(
            0,
            len(game.game_map.get_locations_in_range([-500, -500], 10)),
            "Invalid tiles are being marked as in range",
        )
        self.assertEqual(
            1,
            len(game.game_map.get_locations_in_range([13, 13], 0)),
    ...
    def test_get_units(self):
        game = self.make_turn_0_map()
        self.assertEqual(
            0, len(game.game_map[13, 13]), "There should not be a unit on this location"
        )
        for _ in range(3):
            game.game_map.add_unit("EI", [13, 13])
        self.assertEqual(
            3, len(game.game_map[13, 13]), "Information seems not to be stacking"
        )
        for _ in range(3):
    ...
    def test_get_units_in_range(self):
        ...
        self.assertEqual(
            37,
            len(game.game_map.get_locations_in_range([13, 13], 3.5)),
            "Wrong number of tiles in range",
        )
    ...
    def _test_get_attackers(self):
        ...
        self.assertEqual(
            [], game.get_attackers([13, 13], 0), "Are we being attacked by a ghost?"
        )
        game.game_map.add_unit("DF", [12, 12], 0)
        self.assertEqual(
            [], game.get_attackers([13, 13], 0), "Are we being attacked by a friend?"
        )
        game.game_map.add_unit("EF", [13, 12], 1)
        self.assertEqual(
            [], game.get_attackers([13, 13], 0), "Are we being attacked by a support?"
        )
        game.game_map.add_unit("FF", [14, 12], 1)
        self.assertEqual(
            [], game.get_attackers([13, 13], 0), "Are we being attacked by a wall?"
        )
        game.game_map.add_unit("DF", [12, 14], 1)
        self.assertEqual(
            1, len(game.get_attackers([13, 13], 0)), "We should be in danger"
        )
        game.game_map.add_unit("DF", [13, 14], 1)
        game.game_map.add_unit("DF", [14, 14], 1)
        self.assertEqual(
            3,
            len(game.get_attackers([13, 13], 0)),
            "We should be in danger from 3 places",
        )
    ...
    def test_print_unit(self):
        ...
        expected_string = (
            "Enemy FF, health: 75.0 location: [14, 13] removal:  upgrade: False "
        )
        self.assertEqual(
            got_string,
            expected_string,
            "Expected {} from print_unit test got {} ".format(
    ...
    def future_turn_testing_function(self, game, expected, turns):
        ...
        self.assertAlmostEqual(
            ...
            expected,
            0,
            "Expected {} MP {} turns from now, got {}".format(expected, turns, actual),
        )
